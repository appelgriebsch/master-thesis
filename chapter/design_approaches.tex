...
The main benefits of the Semantic Web approach are the specification of a standardized and generalized format to exchange information on the Web (aka \gls{RDF}) as well as a commonly agreed way to access and query for them (aka \gls{SPARQL}). The \gls{RDF} data format does not only specify the syntax of the information exchanged, but also include the semantic (aka meaning) of them. Due to this fact, resources described in \gls{RDF} format are consistent and semantically self-contained. These characteristics are achieved by providing information in a ``triple''; that is a statement consisting of the resource in question (aka subject), a predicate and the specific value (aka object) for it. To be able to unambiguously identify the meaning of these statements, each part of these statements are usually referring to unique \gls{URI}s. \gls{URI}s can be abbreviated via ``prefix'' definitions to make the whole statements easier to read (see also Section~\ref{sec:semantic_web}). To specify that there is an order 12345 from merchant1, one can come up with the following statement, that uses the explicit schema.org specification of an order: \@
...
\begin{minted}[linenos,
  ...
  @prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
    ...
    <http://www.merchant1.com/orders/12345/> rdfs:type
