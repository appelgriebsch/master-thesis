...
As conclusion one could easily see that there are huge efforts on all participating parties to provide information to or integrate data from each other. As there is no common way to access the information from one of the participants, beside the fundamental \gls{HTTP} protocol and \gls{XML} data format, there have to be a lot of collaborative work between each possible combination of stakeholders upfront for deciding on \gls{API} access patterns and required data structures.
