...
Another kind of query is the CONSTRUCT statement, which is used to retrieve a subgraph from a \gls{RDF} data set, and can also be used to harmonize graphs from different sources that use different schemata. As a query of type CONSTRUCT will return a new \gls{RDF} graph it can also be used for basic reasoning functionality ala ``if this graph-pattern is found, assume that \ldots'', as well as can help resolve issues of identifying entities that are described with different \gls{URI}s \citep[pg. 88-98]{allemang2011semantic}.
